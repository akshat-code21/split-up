...
export function GoogleAuthButton({ className, text = "Continue with Google", ...props }: GoogleAuthButtonProps) {
  return (
    <Button
      variant="outline"
      className={cn(
        "w-full rounded-xl py-3 font-medium transition-all hover:bg-muted/50 hover:text-foreground",
        "border-border bg-transparent shadow-none hover:shadow-sm hover:border-primary/20",
        className,
      )}
      {...props}
      onClick={async() => {
